/// <reference path="typings/chessboard.d.ts" />
...
module TalariusChess {
	export class Game {
		...
		constructor(canvasId: string, scoreBoardId: string, gameId: number, playerColor: string, userId: number) {
			...
			//var canvas = new ChessBoard(canvasId, 'start');
	...
	export class ChessBoardHelper {
		...
		validateMoveEnd(x: number, y: number) {
			...
			var isPawnPromotion: bool = this.isThisAPawnPromotion(this.ChessGame.ChessBoard.ActiveColor, this.ChessGame.ChessBoard.Matrix, this.ChessGame.CurrentMove.StartSquare, this.ChessGame.CurrentMove.EndSquare);

			var isPawnPromotion: bool = this.isThisAPawnPromotion(this.ChessGame.ChessBoard.ActiveColor, this.ChessGame.ChessBoard.Matrix, this.ChessGame.CurrentMove.StartSquare, this.ChessGame.CurrentMove.EndSquare);
			//if (!isPawnPromotion) {
			//	this.Game.HubCom.sendMoveToServer();
			//} else {
			//	this.showPawnPromotionDialog(this.ChessGame.Id, this.ChessGame.ChessBoard.ActiveColor, this.ChessGame.CurrentMove.StartSquare, this.ChessGame.CurrentMove.EndSquare);
			//}
		}
		//pawn promotion
