...
namespace Repository.Dapper.Contrib {
	...
	public static class SqlMapperExtensions {
		...
		private static IEnumerable<PropertyInfo> KeyPropertiesCache(Type type) {
			...
			var keyProperties = allProperties.Where(p => p.GetCustomAttributes(true).Any(a => a is KeyAttribute || a is System.ComponentModel.DataAnnotations.KeyAttribute)).ToList();
			...
			if (keyProperties.Count == 0) {
				...
				if (idProp != null) {
					...
					var idProp2 = allProperties.Where(p => p.Name.ToLower() == type.Name + "_id").FirstOrDefault();
					if (idProp2 != null) {
		...
		public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null, string schema = null) where T : class {
			var type = typeof(T);
			string sql;
			if (!GetQueries.TryGetValue(type.TypeHandle, out sql)) {
				...
				var name = GetTableName(type, schema);
		...
		private static string GetTableName(Type type, string schema = null) {
			string name;
			if (!TypeTableName.TryGetValue(type.TypeHandle, out name)) {
				name = string.IsNullOrEmpty(schema) ? string.Concat("[", type.Name, "]") : string.Concat("[", schema ,"].[", type.Name, "]");
		...
		class ProxyGenerator {
			...
			public static T GetInterfaceProxy<T>() {
				...
				var typeBuilder = moduleBuilder.DefineType(typeOfT.Name + "_" + 0,
